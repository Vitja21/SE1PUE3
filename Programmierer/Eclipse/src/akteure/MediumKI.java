...
public class MediumKI extends KI {
    ...
    public MediumKI(final int nummer) {
        super(nummer);
            ...
            if (next_predictor != null) {
                for (int i = 0; i < next_predictor.size(); i++) {
                    if (next_predictor.get(i) == Collections.min(next_predictor)) {
                        final String ausgabe = this.moeglicherSpielzug(f, this.gegner.get(i));
    ...
    public String moeglicherSpielzug(final Figur f, final Figur gegn) {
    ...
    private boolean hypothese(final Figur f, final Figur gegn, final Point position) {
        final Point p = f.getPosition();
