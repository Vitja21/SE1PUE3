...
public class Spielbrett {
	...
	// imperativ!)
	if (bewegungMoeglichSpielfeld(yOffset, xOffset, eingabeChar)
		&& bewegungMoeglichBelegt(yOffset, xOffset, eingabeChar)
		&& bewegungMoeglichRaster(this.spielobjekte[-eingabeChar[0] + yOffset][eingabeChar[1] + xOffset],
			-eingabeChar[0] + yOffset, eingabeChar[1] + xOffset, -eingabeChar[2] + yOffset,
			eingabeChar[3] + xOffset)) {
    ...
    private boolean bewegungMoeglichBelegt(int yOffset, int xOffset, int[] eingabeChar) {
	boolean moeglich = (this.spielobjekte[-eingabeChar[2] + yOffset][eingabeChar[3] + xOffset] == null
		|| this.spielobjekte[-eingabeChar[2] + yOffset][eingabeChar[3] + xOffset].isEmpty());
	if (!moeglich)
	    fehlermeldung = "Zug auf dieses Feld nicht moeglich. Bereits belegt.";
	return moeglich;
    }

    private boolean bewegungMoeglichSpielfeld(int yOffset, int xOffset, int[] eingabeChar) {
	boolean moeglich = (-eingabeChar[0] + yOffset < this.yLaenge) && ((eingabeChar[1] + xOffset) < this.xLaenge)
