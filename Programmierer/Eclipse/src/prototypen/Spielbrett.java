...
public class Spielbrett {
        ...
        for (int y = 0; y < this.yLaenge; y++) {
            for (int i = 0; i < yFeldLaenge; i++) {
                ...
                for (int x = 0; x < this.xLaenge; x++) {
                    for (int j = 0; j < xFeldLaenge; j++) {
                        output.append(
                                this.generiereSpielobjektSymbolCharArray(this.spielobjekte[y][x], xFeldLaenge,
                                        yFeldLaenge)[i][j]);
                    }
                    output.append(OBEN_UNTEN);
                }
            ...
            if (y >= ((yFeldLaenge / 2) - (ySpielobjektLaenge / 2))
                ...
                for (int x = 0; x < xFeldLaenge; x++) {
                    ...
                    if (x >= ((xFeldLaenge / 2) - (xSpielobjektLaenge / 2))
                            && x <= ((xFeldLaenge / 2) + (xSpielobjektLaenge / 2)
                                    - (1 - (xSpielobjektLaenge % 2)))) {
    ...
    public void bewegen(String eingabe) {
        ...
        // imperativ!)
        if ((-eingabeChar[0] + yOffset < this.yLaenge) &&
                ((eingabeChar[1] + xOffset) < this.xLaenge) &&
                (-eingabeChar[2] + yOffset < this.yLaenge) &&
                ((eingabeChar[3] + xOffset) < this.xLaenge) &&
                (!(eingabeChar[0] == eingabeChar[2] && eingabeChar[1] == eingabeChar[3]))) {
            // Bewegendes Objekt von alt auf neu schieben.
            this.spielobjekte[-eingabeChar[2] + yOffset][eingabeChar[3] + xOffset] = this.spielobjekte[-eingabeChar[0]
                    + yOffset][eingabeChar[1] + xOffset];
